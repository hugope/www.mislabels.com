    ...
    $.fn.extend({
        ...
        locationPicker: function() {
            ...
            return this.each(function() {
                ...
                var searchButton = $("<input class='picker-search-button btn button' type='button' value='Buscar'/>");
                $(this).after(searchButton);

                var picker = $("<div id='" + id + "-picker' class='pickermap'><button class='btn-close button'>Cerrar</button></div>").css({
                    width: options.width,
                ...
                $(this).after(picker);
                var mapDiv = $("<div class='picker-map'>Cargando</div>").css({
                ...
                function findAddress(){
                    var address = $(that).val();
                    if(address == ""){
                        alert("Por favor ingresar una direccion o una Longitud y Latitud.");
                    }else{
                        if(isLngLat(address)){
                            showPicker();
                        }else{
                            geocoder.geocode( {'address': address, 'region': 'uk'}, function(results, status) {
                                if (status == google.maps.GeocoderStatus.OK) {
                                    ...
                                    alert("Codigo de geolocalizacon contiene errores, por la siguiente razon: " + status + ". *Recuerde precionar el boton Buscar despues de escribir la direccion.");
                                }
                            });
                        }
                        $(that).focus();
                ...
                picker.delegate('.btn-close', 'click', function(e){
                    e.preventDefault();
                    e.stopPropagation();
                    picker.fadeOut('fast');
                    // console.log('close placepicker');
                });
                // $('html').click(function() {
                    // picker.fadeOut('fast');
                // });
